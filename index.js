...
client.connect((err) =>{
    const servicesCollection = client.db("heavenly-touch").collection('services')
    const bookingCollection = client.db("heavenly-touch").collection('bookings')
...
app.get('/', (req, res) => {
    res.send('heavenly-touch server')
})

app.listen(port, () => {
    console.log('heavenly-touch server on port', port)
})